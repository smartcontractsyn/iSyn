...
def bar_figure_vec_cosines(ax):
    ...
    with open("data/vec-cosine.txt", "r", encoding="utf-8") as ifile:
        ...
        for line in lines:
            ...
            segs[0] = re.sub('[0-9]', "", segs[0].split("-")[0])
            if segs[0] in cg_dict and cg_dict[segs[0]] in cgs:
                vec_cosines[cgs.index(cg_dict[segs[0]])].append(float(segs[1]))
    ...
    ax.set_ylim(ymin=0.99, ymax=1.003)
...
def draw_figure():
    ...
    bar_figure_vec_cosines(axes)
    plt.yticks(fontsize=10, rotation=45)
    plt.subplots_adjust(left=0.12, bottom=0.16, right=0.98, top=0.97, wspace=0.23, hspace=0.4)
    # plt.yscale("log")
    fig.align_labels()
    plt.show()
